...
function emojileriDonustur(msg, emojiler) {
  for (let emoji in emojiler) {
    if (msg.includes(emoji) || msg.includes(emoji.toUpperCase())) {
      msg = msg.replaceAll(emoji, emojiler[emoji]);
      msg = msg.replaceAll(emoji.toUpperCase(), emojiler[emoji]);
    }
  }
  return msg;
}
